...
const AboutVivek = () => {
            ...
            <>
                {Object.keys(screens).map((screen) => (
                    <div 
                        ...
                        id={screen} 
                        tabIndex="0" 
                        onFocus={() => changeScreen(screen)}
                        onClick={() => changeScreen(screen)}
                        className={(activeScreen === screen ? " bg-ub-orange bg-opacity-100 hover:bg-opacity-95" : " hover:bg-gray-50 hover:bg-opacity-5 ") + " w-28 md:w-full md:rounded-none rounded-sm cursor-default outline-none py-1.5 focus:outline-none duration-100 my-0.5 flex justify-start items-center pl-2 md:pl-2.5"}
                    >
                        <img className=" w-3 md:w-4" alt={`${screen} icon`} src={`./themes/Yaru/status/${screen === 'resume' ? 'download' : screen}.svg`} />
                        <span className=" ml-1 md:ml-2 text-gray-50 ">{screen.charAt(0).toUpperCase() + screen.slice(1)}</span>
                    </div>
                ))}
    ...
    return (
        <div className="w-full h-full flex bg-ub-cool-grey text-white select-none relative">
            ...
            <div onClick={() => setNavbar(!navbar)} className="md:hidden flex flex-col items-center justify-center absolute bg-ub-cool-grey rounded w-6 h-6 top-1 left-1 z-40">
...
// Sub-components (About, Education, Skills, Projects, Resume) follow the original logic but as plain functional components
function About() {
    return (
        <>
...
function Resume() {
    return <iframe className="h-full w-full" src="./files/Vivek-Patel-Resume.pdf" title="vivek patel resume" frameBorder="0"></iframe>;
}